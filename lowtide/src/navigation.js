...
// kosten(p) gewichtet einzelne Felder, ohne sie zu sperren. Ohne das fuhren
// die Streifenwagen quer über Plätze und Grünflächen: 11 von 95 Proben
// während einer Verfolgung lagen neben jeder Fahrbahn. Ein Wagen nimmt den
// Umweg über die Straße, solange er nicht ein Vielfaches länger ist.
// Die Heuristik bleibt zulässig, weil kein Feld weniger als 1 kostet.
export function findPath(start,goal,blocked,step=4,maxNodes=10000,kosten=null){
 ...
 for(let count=0;heap.length&&count<maxNodes;count++){
  ...
  for(const [dx,dz] of [[1,0],[-1,0],[0,1],[0,-1]]){const x=n.x+dx,z=n.z+dz,k=encode(x,z),p=world(x,z),mid=world(n.x+dx/2,n.z+dz/2);if(x<0||z<0||x>Math.max(sx,gx)+12||z>Math.max(sz,gz)+12||blocked(p)||blocked(mid))continue;const g=cost.get(key)+(kosten?kosten(p):1);if(g>=(cost.get(k)??Infinity))continue;cost.set(k,g);parent.set(k,key);push({x,z,f:g+h(x,z)});}
 }
