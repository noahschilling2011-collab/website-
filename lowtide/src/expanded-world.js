...
export class ExpandedWorld extends World{
 ...
 car(color,police=false,c=null){if(!c)return detailedCar(color,police);const d=vehicleTypes[c.model],g=new T.Group();if(['car','pickup'].includes(d.shape)){const form=KAROSSERIE_JE_MODELL[c.model];// Dreckstufe je Fahrzeug, fest an der Kennung: derselbe Wagen ist immer
   ...
   // Nach Form, Farbe und Dreckstufe gecacht: zehn Lacke und sechs Formen
   // ergeben rund sechzig Geometriesätze für 144 Wagen statt 144 eigener.
   backeImModell(m,[m.userData.body,...m.userData.wheels,...m.userData.headlights,...m.userData.taillights,...(m.userData.lights||[])],
    AUTO_CACHE,(form||'limousine')+':'+d.shape);
