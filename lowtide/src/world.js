...
export class World{
 ...
 build(){
  ...
  // Alle Maße sieben Meter nach Osten und vier nach Norden, zusammen mit
  // den Wänden in simulation.js — sonst stünde die Kulisse ohne ihr Gebäude.
  // Alle Maße acht Meter nach Osten und vier nach Norden, zusammen mit
  // den Wänden in simulation.js — sonst stünde die Kulisse ohne ihr Gebäude.
  this.box(-69,.02,-70,41,.1,37,0x758181);for(let z=-87;z<-51;z+=9){this.box(-69,7,z,42,.3,.35,0x3c565e);this.box(-69,6.8,z,8,.15,.25,0xd9e5cd,0,true);}this.text('CALDERA SHIPPING',-69,5.8,-50.4,19,'#bbdcce');this.gateMesh=new T.Group();this.dynbox(this.gateMesh,-69,2.4,-51,8,4.8,.3,0x4b6770);this.scene.add(this.gateMesh);this.disk=new T.Group();this.dynbox(this.disk,-70,.8,-78,3.6,1.5,1.8,0x253c46);this.dynbox(this.disk,-70,1.65,-78,.9,.15,.6,0xe9bd72);this.scene.add(this.disk);this.box(-52,1.6,-90.2,1,1.3,.7,0xdac9a0);this.box(-52,1.8,-90.62,.2,.12,.1,0xe2a467,0,true);this.text('GRID 04',-52,2.5,-90.7,2.2,'#e7c688',Math.PI);
 ...
 setupRain(){this.gun=new T.Group();this.dynbox(this.gun,0,0,.18,.12,.16,.43,0x263138);this.dynbox(this.gun,0,-.12,.04,.11,.2,.12,0x353a3c);this.player.userData.arms[1].add(this.gun);this.gun.position.set(0,-.59,0);this.gun.rotation.x=1.2;// Tropfen als kurze Striche. Als Points waren es bildschirmparallele Quadrate,
  ...
  // 1900 Striche in einem Feld von 74 auf 74 Metern sind 0,35 Tropfen je
  // Quadratmeter — auf dem Standbild ein paar Dutzend lange Striche vor der
  // Stadt. Das Feld ist jetzt 46 Meter breit, also 0,90 je Quadratmeter,
  // und die Striche sind kürzer. Sichtbar wird ohnehin nur, was nah ist;
  // die Hälfte der alten Tropfen fiel hinter der Sichtgrenze.
  const tropfen=2600,a=new Float32Array(tropfen*6);
  for(let i=0;i<tropfen;i++){const x=(this.rng()-.5)*46,y=this.rng()*30,z=(this.rng()-.5)*46,l=.45+this.rng()*.4;
   a.set([x,y,z,x+.2,y+l,z+.12],i*6);}
