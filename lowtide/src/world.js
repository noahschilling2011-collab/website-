...
const MONDLICHT=new T.Color(0x9db6d8);
...
export class World{
 constructor(canvas,sim){this.sim=sim;this.scene=new T.Scene();this.scene.fog=new T.FogExp2(0xc6a7a0,.0038);this.renderer=new T.WebGLRenderer({canvas,antialias:true,powerPreference:'high-performance'});this.renderer.setPixelRatio(Math.min(devicePixelRatio,1.5));this.renderer.shadowMap.enabled=true;this.renderer.shadowMap.type=T.PCFShadowMap;this.renderer.outputColorSpace=T.SRGBColorSpace;this.renderer.toneMapping=T.ACESFilmicToneMapping;this.renderer.toneMappingExposure=1.2;this.sky=new Sky(this.renderer);this.scene.add(this.sky.mesh);this.sonnenRichtung=new T.Vector3();this.blitz=0;this.camera=new T.PerspectiveCamera(58,innerWidth/innerHeight,.15,650);this.camera.position.set(-14,12,98);this.camera.lookAt(-40,3,15);this.groups=new Map();this.rng=random(78);this.hemi=new T.HemisphereLight(0xc4dceb,0x51443e,2.1);this.scene.add(this.hemi);this.sun=new T.DirectionalLight(0xffc995,3);this.sun.position.set(-80,110,20);this.sun.castShadow=true;Object.assign(this.sun.shadow.camera,{left:-130,right:130,top:130,bottom:-130,near:1,far:520});this.sun.shadow.mapSize.set(2048,2048);this.sun.shadow.bias=-.0006;this.sun.shadow.normalBias=.035;this.scene.add(this.sun);this.scene.add(this.sun.target);this.build();this.flush();this.player=this.human(0xe2a062,0x253442);this.scene.add(this.player);this.npcs=sim.npcs.map((n,i)=>{const m=this.human([0xb8bf9f,0x708ba9,0xe2c49c,0xaf6276,0x546a64][i%5],0x303a48);this.scene.add(m);return m;});this.cars=sim.cars.map(c=>{const m=this.car(c.color,false,c);this.scene.add(m);return m;});this.cops=sim.cops.map(()=>{const m=this.car(0xe4e7df,true);this.scene.add(m);return m;});this.contact=this.human(0xd4d2c9,0x242e3c);this.contact.position.set(places.mara.x,0,places.mara.z);this.scene.add(this.contact);this.guard=this.human(0x425164,0x26303c);this.guard.position.set(-73,0,-52);this.scene.add(this.guard);this.marker=new T.Mesh(new T.OctahedronGeometry(.7),new T.MeshBasicMaterial({color:0xeccb80}));this.scene.add(this.marker);this.ring=new T.Mesh(new T.RingGeometry(1.8,2,40),new T.MeshBasicMaterial({color:0xeccb80,side:T.DoubleSide,transparent:true,opacity:.75}));this.ring.rotation.x=-Math.PI/2;this.scene.add(this.ring);this.bulletMeshes=[];this.setupRain();this.resize();}
 box(x,y,z,w,h,d,color=0x999999,rot=0,emissive=false){const key=color+':'+emissive+':'+Math.floor(x/100)+':'+Math.floor(z/100);let g=this.groups.get(key);if(!g){g={material:mat(color,emissive),items:[]};this.groups.set(key,g);}g.items.push({x,y,z,w,h,d,rot});}
 flush(){const o=new T.Object3D();for(const g of this.groups.values()){const mesh=new T.InstancedMesh(cube,g.material,g.items.length);for(let i=0;i<g.items.length;i++){const a=g.items[i];o.position.set(a.x,a.y,a.z);o.scale.set(a.w,a.h,a.d);o.rotation.y=a.rot;o.updateMatrix();mesh.setMatrixAt(i,o.matrix);}mesh.castShadow=true;mesh.receiveShadow=true;mesh.computeBoundingSphere();this.scene.add(mesh);}this.groups.clear();}
 ...
 build(){
  this.box(0,-.6,0,240,1,240,0x857d6f);this.box(158,-1.3,0,85,1,800,0x42696a);this.box(109,-.12,0,9,.2,240,0x9d9583);this.box(114,.45,0,.7,1,240,0xa39d88);
  ...
  const wasser=createWater();this.waterUniforms=wasser.uniforms;
  this.water=new T.Mesh(new T.PlaneGeometry(760,1100,110,150),wasser.material);
  this.water.rotation.x=-Math.PI/2;this.water.position.set(495,-.55,0);this.scene.add(this.water);
  // Der Salzsumpf im Westen war bisher nur eine eingefärbte Geländefläche.
  this.marsh=new T.Mesh(new T.PlaneGeometry(150,155,30,32),wasser.material);
 ...
 update(dt,camYaw,camPitch,playing){const s=this.sim,p=s.player,t=s.time;const himmelJetzt=this.applySky(dt);updateWater(this.waterUniforms,himmelJetzt,t,this.camera);this.rain.visible=s.weather==='rain'||s.weather==='storm';this.rain.material.opacity=s.weather==='storm'?.6:.4;if(this.rain.visible){this.rain.position.set(p.x,0,p.z);const a=this.rain.geometry.attributes.position,fall=dt*(s.weather==='storm'?36:26),h=this.rainHoehe;for(let i=0;i<a.count;i++)a.setY(i,(a.getY(i)-fall+h)%h);a.needsUpdate=true;}
  ...
  if(playing){const dist=p.car?10:6.5;let target=new T.Vector3(p.x,1.6,p.z);let desired=new T.Vector3(p.x-Math.sin(camYaw)*dist,3.7+camPitch*6+(p.car?1.5:0),p.z-Math.cos(camYaw)*dist); // Camera collision against the same world solids.
 ...
 applySky(dt){
  ...
  this.scene.fog.color.copy(himmel.nebel);this.scene.fog.density=himmel.nebelDichte;
  ...
  this.sun.position.set(p.x+richtung.x*190,Math.max(6,richtung.y*190),p.z+richtung.z*190);
  ...
  this.hemi.color.copy(himmel.zenith).lerp(himmel.horizon,.66).multiplyScalar(1.35);
