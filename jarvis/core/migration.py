...
FIX-11 Punkt 1: bis dahin lag das in `scripts/migrate.py`, und niemand rief
es - eine Datenbank von vor FIX-09 gab bei `POST /api/zeitplaene` einen 500
(`no such column: fehlschlaege`), Health sagte trotzdem `ok`. Jetzt laeuft
`migriere()` beim Start (`api/app.py`, lifespan), das Skript bleibt als
Huelle fuer die Hand und den Trockenlauf.
...
def _migriere(conn: sqlite3.Connection, *, dry_run: bool) -> list[str]:
    getan: list[str] = []
...
def migriere(ziel: Path | str | sqlite3.Connection, *,
             dry_run: bool = False) -> list[str]:
    """Zieht eine Datenbank auf den Stand von `core/schema.sql`.

    `ziel` ist der Pfad (Skript, Tests) oder eine offene Verbindung (der
    Start hat sie schon, fuer `quick_check` davor). Mit `dry_run` wird
    nichts geaendert, nur die Liste der Befehle geliefert - leer heisst:
    aktuell. Nach einer Verbindung wird committet; nach einem Pfad
    uebernimmt das `db.session`.
    """
    if isinstance(ziel, sqlite3.Connection):
        return _migriere(ziel, dry_run=dry_run)
    with db.session(ziel) as conn:
        return _migriere(conn, dry_run=dry_run)


def ausstehend(conn: sqlite3.Connection) -> list[str]:
    """Was `migriere` tun wuerde - ohne es zu tun. Der Start sichert vorher,
    wenn hier etwas steht; Health meldet `schema: veraltet`, wenn danach
    noch etwas steht."""
    return migriere(conn, dry_run=True)
